...
class TENERLayer(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        out = torch.einsum('bhij,bjhd->bihd', attn_weight, V)
    ...
    if valid_f1 > valid_f1_prev:
        torch.save({'epoch': epoch, 'model_state': model.state_dict(), 'valid_acc': valid_acc,
            'valid_f1': valid_f1, 'max_seq_length': max_seq_length, 'lower_case': False},
                    os.path.join(output_dir, 'xlnet_tener_crf_dnrti_checkpoint.pt'))
...
output_file = './outputs/dnrti_tener_train.txt'
...
def testModel(model, dataloader, label_list):
    ...
    # 保存预测和真实标签到文件
    output_file = './outputs/dnrti_tener_test.txt'
    os.makedirs(os.path.dirname(output_file), exist_ok=True)
    with open(output_file, 'w') as f:
